...
import { determineGroqMatch } from '../services/groqService';
import { supabase } from '../lib/supabaseClient';
import { mapFirm } from '../lib/services'; // ADDED
...
export const FirmFinderQuiz: React.FC<FirmFinderQuizProps> = ({ onComplete }) => {
  ...
  const processResults = async (finalAnswers: QuizPreferences) => {
    // Call the Groq AI service with the live database and user preferences
    const aiResult = await determineGroqMatch(finalAnswers);

    let winningFirm = MOCK_FIRMS[0]; // Fallback mock

    if (aiResult.firmId) {
      // Fetch the full firm details from Supabase using the ID provided by LLaMA 3
      const { data, error } = await supabase
        .from('firms')
        .select('*, challenges(*)')
        .eq('id', aiResult.firmId)
        .single();

      if (!error && data) {
        // Map the database object securely using the shared utility
        winningFirm = mapFirm(data) as any;
      }
    }

    setResult({ firm: winningFirm, explanation: aiResult.explanation });
