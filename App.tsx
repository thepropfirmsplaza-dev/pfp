...
export default function App() {
  ...
  const renderContent = () => {
    switch(currentView) {
      case 'home':
        return <Hero onStartQuiz={() => setView('quiz')} onExplore={() => setView('discovery')} />;
        ...
        return (
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
                <div className="flex justify-between items-end mb-10">
                    <div>
                        <h2 className="text-4xl font-bold mb-2">Firm Comparison</h2>
                        <p className="text-text-muted">Analyze features, spreads, and rules side-by-side.</p>
                    </div>
                </div>
                <ComparisonTable firms={selectedFirms} onRemove={(id) => setSelectedFirms(selectedFirms.filter(f => f.id !== id))} />
                {selectedFirms.length === 0 && (
                     <div className="mt-8 text-center">
                        <button onClick={() => setView('discovery')} className="btn-primary px-8 py-3 rounded-full text-sm font-bold shadow-lg hover:shadow-xl transition-all">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            {selectedFirms.length > 0 && (
                <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-40 bg-dark-mid/80 backdrop-blur-xl border border-white/10 rounded-full px-8 py-4 shadow-2xl flex items-center space-x-6 animate-fade-in ring-1 ring-white/10">
                    ...
                    <button onClick={() => setView('compare')} className="text-sm font-bold text-primary hover:text-white transition-colors uppercase tracking-wider">
                        Compare Now
                    </button>
